...
cont = w3.eth.contract(
    address='0x5fCA3dE616944C0B3135b6C5e853d81eBE8712E3',
    abi=abi_json
)

def create(upda,file_name,path,sa):
  md5 = hashlib.md5()
  sha1 = hashlib.sha1()
  BUF_SIZE = 65536 
  histogram=[]
  ...
  if sa =='0':
    ...
    histogram,DCT_val=DCT.get_imagedata("planet.jpg")
    #print(DCT_val)
  else:
    thi="empty"
  cont.functions.createtask(thi,file_name,upda,path).transact()
  #print(last_update)
  return

def check(upda,file_name,path,sa):
  md5 = hashlib.md5()
  sha1 = hashlib.sha1()
  BUF_SIZE = 65536 
  #check if not empty array 4
  if sa !='0':
    ...
    thi="empty"
  x=cont.functions.check(thi,file_name,upda,path).call()
  if x==False:
